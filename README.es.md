<p align="right">
  <a href="./README.md">🇺🇸 English</a> | <a href="./README.pt-BR.md">🇧🇷 Português</a> | <b>🇪🇸 Español</b>
</p>

<h1 align="center">Hola 👋, soy Guilherme Damasio</h1>
<p align="center">Desarrollador back-end en formación, enfocado en C# & .NET</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=guilhermedamasio&label=Visitas%20al%20perfil&color=0e75b6&style=flat" alt="Visitas al perfil" />
  <img src="https://img.shields.io/github/followers/guilhermedamasio?label=Seguidores&style=flat" alt="Seguidores" />
</p>

<table align="center" width="100%">
  <tr>
    <td width="50%" align="center">
      <img
        src="https://github-stats-extended.vercel.app/api?username=guilhermedamasio&show_icons=true&include_all_commits=true&theme=tokyonight&hide_border=true&rank_icon=github"
        alt="Estadísticas de GitHub"
        width="100%"
      />
    </td>
    <td width="50%" align="center">
      <img
        src="https://github-readme-streak-stats-eight.vercel.app/?user=guilhermedamasio&theme=tokyonight&hide_border=true"
        alt="Racha de GitHub"
        width="100%"
      />
    </td>
  </tr>
</table>

<h2 align="center">🛠️ Tecnologías</h2>
<p align="center">
  <img
    src="https://skillicons.dev/icons?i=cs,dotnet,py,js,html,css,mysql,azure,git,github,docker,visualstudio,vscode,linux&perline=18&size=45"
    alt="Tech Stack"
  />
</p>

<table align="center" width="100%">
  <tr>
    <td width="75%" align="center">
      <h3>📈 Actividad de contribuciones</h3>
      <img
        src="https://github-readme-activity-graph.vercel.app/graph?username=guilhermedamasio&theme=tokyo-night&hide_border=true&area=true&height=180"
        alt="Actividad de contribuciones"
        width="100%"
      />
    </td>
    <td width="25%" align="center">
      <h3>📌 Resumen de GitHub</h3>
      <img
        src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=guilhermedamasio&theme=tokyonight"
        alt="Resumen de GitHub"
        width="100%"
      />
    </td>
  </tr>
</table>

<table align="center" width="100%">
  <tr>
    <td width="50%" align="center">
      <img
        src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=guilhermedamasio&theme=tokyonight"
        alt="Lenguajes por repositorio"
        width="100%"
      />
    </td>
    <td width="50%" align="center">
      <img
        src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=guilhermedamasio&theme=tokyonight"
        alt="Lenguajes por commit"
        width="100%"
      />
    </td>
  </tr>
</table>

<h2 align="center">📌 Proyectos destacados</h2>
<table align="center" width="100%">
  <tr>
    <td width="33%" align="center" valign="top">
      <b>Control de Gastos</b><br/>
      <sub>App en C# con categorización de gastos mediante enum y control de límite de tarjeta. Evolucionando hacia una API.</sub>
    </td>
    <td width="33%" align="center" valign="top">
      <b>ToDo App</b><br/>
      <sub>Gestor de tareas en C#, con planes de persistencia en base de datos.</sub>
    </td>
    <td width="33%" align="center" valign="top">
      <b>Validador de CPF</b><br/>
      <sub>Validador de identificación fiscal brasileña (CPF) en C# con categorización mediante enum (beta).</sub>
    </td>
  </tr>
</table>
<p align="center"><a href="https://github.com/guilhermedamasio?tab=repositories">Ver todos los repositorios →</a></p>

<h2 align="center">📫 ¿Hablamos?</h2>
<p align="center">
  <a href="https://linkedin.com/in/guilhermedamasio09">
    <img src="https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="mailto:guilhermedamasiomendes@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white" alt="Email" />
  </a>
</p>
